...
public class AmountInWordsConverter {
    ...
    public static String convert(BigDecimal amount) {
        ...
        long integralPart = amount.longValue();
        int fractionalPart = amount.remainder(BigDecimal.ONE).movePointRight(2).intValue();

        String integralWords = convertNumber(integralPart);
        String fractionalWords = fractionalPart > 0 ? " and " + convertNumber(fractionalPart) + " Paise" : "";

        return integralWords + " Rupees" + fractionalWords + " Only";
    }
