...
public class OpdVisitService {
    ...
    public OpdVisitDto createOpdVisit(CreateOpdVisitRequest request) {
        ...
        return mapToOpdVisitDto(savedVisit, bill);
    }
    ...
    public BillDto addServiceToBill(String billId, AddServiceToBillRequest request) {
        ...
        if (bill.getStatus() != BillStatus.DRAFT) {
            throw new RuntimeException(
                    "Cannot add services to a " + bill.getStatus() + " bill. Revert to DRAFT or create new visit.");
        }
    ...
    public BillDto recalculateBill(Bill bill) {
        ...
        bill.setGrandTotal(
                subTotal.add(bill.getTax()).subtract(bill.getDiscount()).setScale(2, java.math.RoundingMode.HALF_UP));

        bill.setAmountInWords(AmountInWordsConverter.convert(bill.getGrandTotal()));

        Bill savedBill = billRepository.save(bill);
        return mapToBillDto(savedBill, items, paymentRepository.findByBill_BillId(bill.getBillId()));
    }
    ...
    public BillDto addPayment(String billId, CreatePaymentRequest request) {
        ...
        Bill savedBill = billRepository.save(bill);
        return mapToBillDto(savedBill, billItemRepository.findByBill_BillId(billId), allPayments);
    }
    ...
    private BillDto mapToBillDto(Bill bill, List<BillItem> items, List<Payment> payments) {
        return BillDto.builder()
                ...
                .patientName(
                        bill.getVisit().getPatient().getFirstName() + " " + bill.getVisit().getPatient().getLastName())
    ...
    private PaymentDto mapToPaymentDto(Payment payment) {
        return PaymentDto.builder()
                ...
                .receivedBy(payment.getReceivedBy() != null ? payment.getReceivedBy().getUsername() : "SYSTEM")
                .build();
    }
}
